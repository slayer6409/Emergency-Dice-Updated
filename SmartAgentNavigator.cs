...
namespace MysteryDice
{
    public class SmartAgentNavigator : NetworkBehaviour
    {
        ...
        public void SetAllValues(bool isOutside)
        {
            ...
            exitPoints.Clear();
            foreach (var exit in FindObjectsByType<EntranceTeleport>(FindObjectsInactive.Exclude,
        ...
        public bool DoPathingToDestination(Vector3 destination, bool destinationIsInside, bool followingPlayer,
            PlayerControllerB? playerBeingFollowed)
        {
            if (!agent.enabled)
            {
                ...
                float normalizedDistance =
                    Mathf.Clamp01(Vector3.Distance(transform.position, targetPosition) / distanceToTarget);
                Vector3 newPosition = Vector3.MoveTowards(transform.position, targetPosition,
                    Time.deltaTime * nonAgentMovementSpeed);
                ...
                {
                    child.position = transform.position + child.localPosition;
                }
            ...
            {
                bool scriptCloserToTop =
                    Vector3.Distance(transform.position, elevatorScript.elevatorTopPoint.position) <
                    Vector3.Distance(transform.position, elevatorScript.elevatorBottomPoint.position);
                bool destinationCloserToTop = Vector3.Distance(destination, elevatorScript.elevatorTopPoint.position) <
                                              Vector3.Distance(destination,
                                                  elevatorScript.elevatorBottomPoint.position);
            ...
            bool playerIsInElevator = elevatorScript != null && !elevatorScript.elevatorFinishedMoving &&
                                      Vector3.Distance(destination, elevatorScript.elevatorInsidePoint.position) < 7f;
        ...
        private bool DetermineIfNeedToDisableAgent(Vector3 destination)
        {
            ...
            if ((!agent.CalculatePath(destination, path) || path.status == NavMeshPathStatus.PathPartial) &&
                Vector3.Distance(transform.position, destination) > 7f)
            {
            ...
            if (followingPlayer && playerBeingFollowed != null)
            {
                ...
                foreach (var exitpoint in exitPoints.Keys)
                {
                    if (closestExitPointToPlayer == null ||
                        Vector3.Distance(positionOfPlayerBeforeTeleport, exitpoint.transform.position) <
                        Vector3.Distance(positionOfPlayerBeforeTeleport, closestExitPointToPlayer.transform.position))
                    {
            ...
            else
            {
                if (lastUsedEntranceTeleport != null)
                {
                    ...
                    destination = !isOutside
                        ? lastUsedEntranceTeleport.exitPoint.transform.position
                        : lastUsedEntranceTeleport.entrancePoint.transform.position;
                ...
                else
                {
                    ...
                    foreach (var exitpoint in exitPoints.Keys)
                    {
                        if (closestExitPointToScript == null ||
                            Vector3.Distance(this.transform.position, exitpoint.transform.position) <
                            Vector3.Distance(this.transform.position, closestExitPointToScript.transform.position))
                        {
        ...
        private bool NeedsElevator(Vector3 destination, EntranceTeleport entranceTeleportToUse,
            MineshaftElevatorController elevatorScript)
        {
            // Determine if the elevator is needed based on destination proximity and current position
            bool nearMainEntrance = Vector3.Distance(destination, RoundManager.FindMainEntrancePosition(true, false)) <
                                    Vector3.Distance(destination, entranceTeleportToUse.transform.position);
            bool closerToTop = Vector3.Distance(transform.position, elevatorScript.elevatorTopPoint.position) <
                               Vector3.Distance(transform.position, elevatorScript.elevatorBottomPoint.position);
        ...
        private void UseTheElevator(MineshaftElevatorController elevatorScript)
        {
            ...
            bool goUp = Vector3.Distance(transform.position, elevatorScript.elevatorBottomPoint.position) <
                        Vector3.Distance(transform.position, elevatorScript.elevatorTopPoint.position);
        ...
        {
            return !elevatorScript.elevatorCalled && ((!elevatorScript.elevatorIsAtBottom && needToGoUp) ||
                                                      (elevatorScript.elevatorIsAtBottom && !needToGoUp));
        }
        ...
        private void MoveToWaitingPoint(MineshaftElevatorController elevatorScript, bool needToGoUp)
        {
            ...
            if (Vector3.Distance(transform.position, elevatorScript.elevatorInsidePoint.position) > 1f)
            {
                agent.SetDestination(needToGoUp
                    ? elevatorScript.elevatorBottomPoint.position
                    : elevatorScript.elevatorTopPoint.position);
            }
            else
            {
        ...
        private IEnumerator SearchAlgorithm(Vector3 position, float radius)
        {
            while (isSearching)
            {
                ...
                while (!reachedDestination && isSearching)
                {
                    ...
                    if (Vector3.Distance(this.transform.position, positionToTravel) <= 10f ||
                        agent.velocity.magnitude <= 1f)
                    {
