...
public class GalAI : NetworkBehaviour, IHittable
{
    ...
    {
        MysteryDice.ExtendedLogging($"Entered Elevator: {enteredElevator}");
    }

    public virtual void OnEnableOrDisableAgent(bool agentEnabled)
    {
        MysteryDice.ExtendedLogging($"Enabled Agent: {agentEnabled}");
    }

    public virtual void OnUseEntranceTeleport(bool setOutside)
    {
        MysteryDice.ExtendedLogging($"Used Entrance Teleport and should be set outside: {setOutside}");
    ...
    public void SetEnemyTargetClientRpc(NetworkBehaviourReference networkBehaviourReference)
    {
        ...
        MysteryDice.ExtendedLogging($"{this} setting target to: {targetEnemy.enemyType.enemyName}");
    }
