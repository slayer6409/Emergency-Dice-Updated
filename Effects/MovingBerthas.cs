...
namespace MysteryDice.Effects
{
    internal class MovingSeaTraps : IEffect
    {
        ...
        public static IEnumerator WaitForSeaTrapInit()
        {
            ...
            {
                if (mine.transform.parent.gameObject.GetComponent<BerthaMovement>() == null)
                {
                    mine.transform.parent.gameObject.AddComponent<BerthaMovement>().bigbertha = mine;
                }
            }

            foreach (var mine in GameObject.FindObjectsOfType<Seamine>())
            {
                if (mine.transform.parent.gameObject.GetComponent<SeamineMovement>() == null)
                {
                    mine.transform.parent.gameObject.AddComponent<SeamineMovement>().seamine = mine;
                }
            }
        }
            ...
            public void FixedUpdate()
            {
                ...
                if(Networker.Instance.IsServer)
                {
                    ...
                    if (PathTimer <= 0f)
                    {
                        ...
                        Networker.Instance.SyncDataClientRPC(seamine.NetworkObjectId,MoveSpeed, transform.position, TargetPosition, BlockedID);
                    }
                }
            ...
            public void FixedUpdate()
            {
                ...
                if(Networker.Instance.IsServer)
                {
                    ...
                    if (PathTimer <= 0f)
                    {
                        ...
                        CalculateNewPath();
                        MoveSpeed = UnityEngine.Random.Range(3f, 7f);
                        Networker.Instance.SyncDataClientRPC(bigbertha.NetworkObjectId,MoveSpeed, transform.position, TargetPosition, BlockedID);
                    }
                }
