...
{
    internal class BeegCrane : IEffect
    {
        public string Name => "Beeg Crane";
        ...
        public string Tooltip => "The sky is falling";
        public void Use()
        {
            Networker.Instance.SpawnBeegCraneServerRPC();
        }

        public static void SpawnBeegCrane(int MinesToSpawn, float positionOffsetRadius = 5f)
        {
            int spawnedMines = 0;
                        ...
                        if (Physics.Raycast(randomPosition + Vector3.up * 10, Vector3.down, out RaycastHit hit, 20f))
                        {
                            ...
                            if (GetShortestDistanceSqr(groundPosition, allPositions) >= 1)
                            {
                                ...
                                GameObject gameObject = UnityEngine.Object.Instantiate(
                                    DynamicTrapEffect.getTrap("Autonomous Crane").prefab,
                                    groundPosition,
                                    Quaternion.identity,
                                ...
                                spawnedMines++;
                                Networker.Instance.setSizeClientRPC(netobj.NetworkObjectId,new Vector3(2f,2f,2f));
                            }
                        }
                    }
