...
{
    internal class SmolCrane : IEffect
    {
        public string Name => "Smol Crane";
        ...
        public string Tooltip => "it smol";
        public void Use()
        {
            Networker.Instance.SpawnSmolCraneServerRPC();
        }

        public static void SpawnSmolCrane(int MinesToSpawn, float positionOffsetRadius = 5f)
        {
            int spawnedMines = 0;
                    ...
                    for (int attempt = 0; attempt < maxAttempts && !validPositionFound; attempt++)
                    {
                        ...
                        if (Physics.Raycast(randomPosition + Vector3.up * 10, Vector3.down, out RaycastHit hit, 20f))
                        {
                            ...
                            if (GetShortestDistanceSqr(groundPosition, allPositions) >= 1)
                            {
                                ...
                                spawnedMines++;
                                Networker.Instance.setSizeClientRPC(netobj.NetworkObjectId,new Vector3(0.5f,0.5f,0.5f));
