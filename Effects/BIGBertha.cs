...
{
    internal class BIGBertha : IEffect
    {
        public string Name => "BIG Bertha";
        ...
        public void Use()
        {
            Networker.Instance.BIGBerthaServerRPC();
        }

        public static void SpawnBIGBertha(int MinesToSpawn, float positionOffsetRadius = 5f)
        {
            int spawnedMines = 0;
                    ...
                    for (int attempt = 0; attempt < maxAttempts && !validPositionFound; attempt++)
                    {
                        ...
                        if (Physics.Raycast(randomPosition + Vector3.up * 10, Vector3.down, out RaycastHit hit, 20f))
                        {
                            ...
                            if (GetShortestDistanceSqr(groundPosition, allPositions) >= 1)
                            {
                                ...
                                spawnedMines++;
                                Networker.Instance.setSizeClientRPC(netobj.NetworkObjectId,new Vector3(100,100,100));
