...
namespace MysteryDice.Effects
{
    internal class ScrapJackpot : IEffect
    {
        ...
        public static void JackpotScrap(int userID, int amount)
        {
            ...
            {
                scrapToSpawn.Add(RM.currentLevel.spawnableScrap[RM.GetRandomWeightedIndex(weights)].spawnableItem);
            }
        ...
        public static IEnumerator DelayedSync(RoundManager RM, NetworkObjectReference[] netObjs, int[] scrapValues, float[] scrapWeights)
        {
            ...
            RM.SyncScrapValuesClientRpc(netObjs, scrapValues); // Sync to clients
            Networker.Instance.SyncItemWeightsClientRPC(netObjs, scrapWeights); // Sync weights
        }
    }
}
